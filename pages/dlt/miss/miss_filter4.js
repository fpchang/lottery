...
const historyRedData =dltHistory.map(item=>item.redBall);
/**
 * 双色球红球全组合筛选工具
...
class DoubleColorBallRedFilter {
  ...
  validateHistoryRedData(data) {
    ...
    data.forEach((combo, index) => {
      ...
      if (!Array.isArray(combo) || combo.length !== 5) {
        throw new Error(`第${index+1}期红球数据错误：必须包含5个数字`);
      }

      // 验证每个数字：1-35、整数、不重复
      const validNums = combo.filter(num => num >= 1 && num <= 35 && Number.isInteger(num));
  ...
  generateCombinations(n, k, progressCallback) {
    const total = this.calculateCombination(35, 5); // 总组合数：1107568
    let currentCount = 0;
    const result = [];
  ...
  calculateAllValidCombos(progressCallback) {
    const totalCombos = this.calculateCombination(35, 5);
    ...
    // 生成并筛选所有组合
    this.validCombos = this.generateCombinations(35, 5, wrappedProgress);
  ...
  exportResults() {
    return this.validCombos
      .map((combo, index) => `组合${index+1}：${combo}`)
      .join("\n");
  }
}
...
try {
  ...
  validCombos.filter(item=>item[0]==3&&item[4]<25).forEach((combo, index) => {
